...
<html lang="en">
  <head>
    ...
    <title>Puffers Activation Page</title>
...
<!--           Innercontent-->
         <div id="innercontent">
                 <div class="introtext">
                           <h1 style="text-align: center">Let's get you puffing!</h1>
                            <!--// //////////////////////////PHP CODE GOES HERE//////////////////////////-->
                            <?php
                            ...
                            //var_dump($link);
                            //   >>>>>>> check if the datas (USERNAME AND ACTIVATION CODE) are present
                            //    Get them if they are
                            // Define  some possible errors
                            $errors = "<div class='alert alert-danger'>Invalid activation link. Please click the link mailed to your provided Gmail address. Contact support if the error persists. Thanks!</div>";
                            $error2 = "<div class='alert alert-danger'>Unable to run UPDATE query</div>";
                            if(isset($_GET["username"])){
                                $usnm = $_GET["username"];
                            }else{
                            //    Show error message if absent/
                                echo $errors;
                            }
                            ...
                            //    ready the datas
                                $username = mysqli_real_escape_string($link, $usnm);
                                $firstkey = mysqli_real_escape_string($link, $activationkey);
//                     echo $username;
//                     echo $
                            //    update users table by setting activation to activated
                     $sql = "UPDATE users SET activation='activated' WHERE (username='$username' AND activation='$firstkey')";
                     $result = mysqli_query($link, $sql);
                     if(!$result){
                         echo $error2;
                         exit;
                     }   
                     // set a variable to be equal to number of rows affected
                     $jiii = mysqli_affected_rows($link);
                     if($jiii == 1){
                             echo "<div class='alert alert-success'>Your account has been successfully activated. Please proceed to Puff-in</div>";
                         echo "<div class='btn btn-lg btn-info'><a href='index.php'>Puff-in</a></div>";
                         }else{
                         echo "<div class='alert alert-danger'>Sorry, we are unable to activate your account at this moment. Please contact support <a href='contactus.php' class='btn-info btn'>here</a></div>";
                     }
                         
                     
                            //    show success message

// Sample activation code mailed to successfully registered users
//http://localhost/puffersarena/activate.php?activationkey=37ef3b34a3fe1bdce057e03c736e8f02&username=kENKENfdfe
                            ?>
